...
function drawPart() {
    ...
    for(i = 0; i < num_parts; i++) {
        ctx.beginPath();
        ctx.rect(myCanvas_x_start, myCanvas_y_start + part_start[i]*(700/total_mem_size), myCanvas_width, part_size[i]*(700/total_mem_size));
        ctx.fillStyle = "#FFD600";
        ctx.fill();

        ctx.font = "14px Arial bold";
        ctx.fillStyle = "black";
        ctx.fillText("P"+ String(part_pro_id[i]) + " (" + String(part_size[i]) + "B)", 50, myCanvas_y_start + part_start[i]*(700/total_mem_size) + part_size[i]*(700/total_mem_size)/2);
    }
}
