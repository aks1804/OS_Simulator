    ...
    for(i = 0; i < num_parts; i++) {
        if(part_occupied[i] == 0 && found == 0) {
            if(pro_size <= part_size[i]) {
                ...
                // ctx.beginPath();
                // ctx.rect(myCanvas_x_start, part_myCanvas_start[i], myCanvas_width, pro_size*(700/total_mem_size));
                // ctx.fillStyle = "green";
                // ctx.fill();

                // ctx.font = "14px Arial bold";
                // ctx.fillStyle = "black";
                // ctx.fillText("P-"+ String(pro_id), myCanvas_width/2, part_myCanvas_start[i] + pro_size*(700/total_mem_size)/2);
            }
        }
    }
    if(found == 0 && fromQ == 0) {
